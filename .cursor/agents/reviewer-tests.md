...
```bash
git diff master...HEAD --name-only -- "*.ts" "*.tsx" | grep -v "\.spec\."
```
...
- `frontend/src/foo.ts` → `frontend/src/foo.spec.ts`
- `frontend/src/components/bar.tsx` → `frontend/src/components/bar.spec.tsx`
